...
use crate::{prelude::UiCameraConfig, BackgroundColor, BorderColor, CalculatedClip, Node, UiImage};
use crate::{ContentSize, Style, Val};
use crate::{UiStackIndex, UiTextureAtlasImage};
...
pub fn extract_uinodes(
    ...
    for (&UiStackIndex(stack_index), uinode, transform, color, maybe_image, visibility, clip) in
        uinode_query.iter()
    {
        // Skip invisible and completely transparent nodes
        if !visibility.is_visible() || color.0.a() == 0.0 {
