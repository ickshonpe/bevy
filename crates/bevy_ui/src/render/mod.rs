...
use crate::{UiPosition, ZIndex, UiStackIndex};
...
impl ExtractedUiNodes {
    ...
    fn sort_ranges(&mut self) {
        self.ranges.sort_by_key(|range| range.stack_index);
    }
...
pub fn extract_uinodes(
    ...
    for (
        uinode, position, color, maybe_image, visibility, clip, z) in uinode_query.iter() {
...
pub fn extract_text_uinodes(
    ...
    for (uinode, position, text, text_layout_info, visibility, clip, z) in uinode_query.iter() {
            ...
            let transform = Affine2::from_translation(position.0 - 0.5 * uinode.size());
...
pub fn prepare_uinodes(
    ...
    for extracted_range in &extracted_uinodes.ranges {
