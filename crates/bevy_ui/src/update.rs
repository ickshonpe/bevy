...
use crate::{CalculatedClip, OverflowAxis, Style, UiPosition};
...
pub fn update_clipping_system(
    ...
    mut node_query: Query<(&Node, &UiPosition, &Style, Option<&mut CalculatedClip>)>,
...
fn update_clipping(
    ...
    children_query: &Query<&Children>,
    node_query: &mut Query<(&Node, &UiPosition, &Style, Option<&mut CalculatedClip>)>,
    entity: Entity,
    ...
    let children_clip = if style.overflow.is_visible() {
        ...
        let mut node_rect = node.logical_rect(position);
