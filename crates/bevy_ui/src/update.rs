...
pub fn update_clipping_system(
    ...
    mut node_query: Query<
        (&Node, &GlobalTransform, &Style, Option<&mut CalculatedClip>),
        Without<Style<()>>,
    >,
    mut content_node_query: Query<
        (
            &Node,
            &GlobalTransform,
            &Style<()>,
...
fn update_clipping(
    ...
    if let Ok((node, global_transform, style, maybe_calculated_clip)) = node_query.get_mut(entity) {
        ...
        let (node, global_transform, style, maybe_calculated_clip) =
            content_node_query.get_mut(entity).unwrap();
