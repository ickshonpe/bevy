                ...
                match &extracted_uinode.item {
                    ExtractedUiItem::Node {
                        ...
                        let mut positions_diff = if let Some(clip) = extracted_uinode.clip {
                            ...
                            //     + Vec2::new(hw - clip.half_size().x, hh - clip.half_size().y);
                            let clip = Rect::from_center_half_size(clip.center(), (hw, hh).into());
                            [
                                Vec2::new(
                    ...
                    ExtractedUiItem::Glyphs { range } => {
                        ...
                        for glyph in &extracted_uinodes.glyphs[range.clone()] {
                            ...
                            let positions_diff = if let Some(clip) = extracted_uinode.clip {
                                ...
                                //     + Vec2::new(hw - clip.half_size().x, hh - clip.half_size().y);
                                let clip =
                                    Rect::from_center_half_size(clip.center(), (hw, hh).into());
                                [
                                    Vec2::new(
                                        f32::max(clip.min.x - positions[0].x, 0.),
                                        f32::max(clip.min.y - positions[0].y, 0.),
